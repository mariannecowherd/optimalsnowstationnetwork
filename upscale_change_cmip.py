...
# read CMIP6 files
mrso_hist = xr.open_dataset(f'{largefilepath}mrso_historical.nc')
mrso_fut = xr.open_dataset(f'{largefilepath}mrso_ssp585.nc')
pred_hist = xr.open_dataset(f'{largefilepath}pred_historical.nc')
pred_fut = xr.open_dataset(f'{largefilepath}pred_ssp585.nc')

# merge historical and future scenarios
mrso = xr.concat([mrso_hist, mrso_fut], dim='time')
pred = xr.concat([pred_hist, pred_fut], dim='time')

# select timerange of ismn
mrso = mrso.sel(time=slice('1960','2050'))
pred = pred.sel(time=slice('1960','2050'))

# read station data
stations = xr.open_dataset(f'{largefilepath}df_gaps_cmip.nc')['mrso']
...
timerange = pd.date_range('2021-01-01','2051-01-01', freq='M')
tmp = stations[:len(timerange),:]
tmp[:,:] = np.nan
tmp['time'] = timerange
tmp[:,~iold] = 1
...
for lat, lon in zip(stations.lat_cmip, stations.lon_cmip):
    ...
    #if landmask.loc[point.lat,point.lon].item(): # obs gridpoint if station contained and on CMIP land # this is ensured by preproc_ismn.py
