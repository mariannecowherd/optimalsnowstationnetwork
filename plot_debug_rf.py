...
meancorr = xr.DataArray(np.full((4,3,len(modelnames),25), np.nan), 
...
# test configs
testcases = ['_sqrt','_sqrt_trees','_sqrt_trees_oldcorr']
testcol = ['blue','green','black']
strategy = 'systematic'
modelname = 'MIROC6'
for metric, ax in zip(metrics, (ax1,ax2,ax3,ax4)):
    for t, testcase in enumerate(testcases):
        try:
            with open(f"corr_{strategy}_{modelname}_{metric}{testcase}.pkl", "rb") as f:
                corr = pickle.load(f)

            with open(f"nobs_{strategy}_{modelname}_{metric}{testcase}.pkl", "rb") as f:
                nobs = pickle.load(f)
        except FileNotFoundError:
            continue
        else:
        ...
        # plot
        ax.plot(frac, corr, c=testcol[t], linewidth=0.5)
# test configs end
...
ax1.vlines(frac_harmonised[0], ymin=0.1, ymax=1.1, colors='grey') # TODO correct?
